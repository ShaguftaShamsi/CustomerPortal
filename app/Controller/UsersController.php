...
class UsersController extends AppController{
  ...
  public $components =array('Manager','EmailManager','Cookie','Captcha');
  ...
  public function index(){
     ...
     $this->User->set($this->request->data);
      if (!$this->User->validates(array('email'))) {
 ...
 //$this->set('myCookie',$this->Cookie->read('users_cookie'));
  ...
  public function login(){
    ...
    if(!$this->Manager->checkSession()){
	    //$this->User->set($this->request->data);
		//if($this->User->validates(array('email'))){
      $this->Session->write('tempEmail.email' ,$this->data['User']['email']);
     if($this->request->is('post')){
      $this->User->set($this->request->data);
      if (!$this->User->validates(array('email'))) {
      $errors = $this->User->validationErrors;
 ...
 //$this->set('myCookie',$this->Cookie->read('users_cookie'));
  ...
  public function forgetPassword(){
    ...
    if(!($this->Manager->read())){
	   // $email=$this->Session->read('tempEmail.email');
		//print_r($email);
     if($this->Session->check('tempEmail')){
         $email=$this->Session->read('tempEmail.email');
         ...
         if(!empty($result)){
           ...
           $this->User->id = $id;
            if($this->User->saveField('reset_pass_token',$hashToken)){
              ...
              $errors['emailMessage'] = 'Something went wrong.Please try again.';
              }
           }
         else{
